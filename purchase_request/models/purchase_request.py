...
class PurchaseRequest(models.Model):
    ...
    def button_change_state_to_rejected(self):
      for order in self:
        if order.state not in ['submit','approved']:
            continue
        else:
          if not order.reject_reason:
            ...
            self.write({'reject_reason':'','state':'rejected','submit_email':''})
...
class InheritPurchase(models.Model):
    ...
    state = fields.Selection([
        ...
        ('purchase', 'Employee Manager Approval'),
        ('approval1', 'Purchase Manager Approval'),
        ('approval2', 'Finanace Manager Approval'),
        ('approval3', 'General Manager Approval'),
        ('approval4', 'Final Approval'),
        ('reject', 'Rejected'),
        ('done', 'Confirm'),
        ('cancel', 'Cancelled')
    ], string='Status', readonly=True, index=True, copy=False, default='draft', track_visibility='onchange')
      ...
      for order in self:
        if order.state not in ['submit','purchase']:
          ...
          if not order.reject_reason:
            ...
            template.write({'email_to': emails})
            template.send_mail(self.id,force_send=True)
            self.write({'reject_reason':'','state':'reject','first_email':'','second_email':''})
