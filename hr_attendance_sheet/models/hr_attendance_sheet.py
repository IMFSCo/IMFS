...
class attendance_sheet(models.Model):
    ...
    def onchange_employee(self):
        ...
        print('the contracts is ',contract_ids)
        if not contract_ids:
            return
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            def get_work_intervals(calender, day_start, day_end):
                ...
                working_intervals = []
                for att in calender._get_day_attendances(day_start.date(),day_start.replace(hour=0, minute=0, second=0).time(),day_end.time()):
