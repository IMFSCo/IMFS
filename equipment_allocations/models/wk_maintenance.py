...
class AllocationRequest(models.Model):
    ...
    def get_date(self, sign_in):
        return self.get_sign_in_time(sign_in)
...
#        dif2 = datetime.strftime(dif,'%d-%m-%Y')
        time_start = sign_in + str(dif)
        return time_start
                ...
                if obj.state in ["cancel", 'returned']:
                    ...
                    if obj.state == 'cancel':
                        ...
                        if cancel_template:
                            ...
                            body_html = Template.with_context(cancel_template._context).render_template(
    ...
    def employee_allocated_history(self):
        employees = self.env['hr.employee'].search([('staff_id','=',self.req_emp_id)])
        if not employees:
            return
        else:
